...
- **DONE:** Migrated off ChatGPT's "Sites" builder into a clean static site here on Desktop
  ...
  no console errors. Under its own git repo.
...
1. **Domain:** register **davislandworksva.com** (davislandworks.com is taken — registered 2009,
   just lapsed to a GoDaddy expired-park as of ~9/13; alternatives all open). After registration:
   add a `CNAME` file here and point DNS at GitHub Pages (apex A records
   185.199.108–111.153, or a `www` CNAME to `bylerent-bot.github.io`).
